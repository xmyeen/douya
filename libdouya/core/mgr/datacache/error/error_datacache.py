...
class ErrorDatacacheObjectBuilder(IDatacacheObjectBuilder):
    def __init__(self, error_id:int, error_entry : dict[str, typing.Any]):
    ...
    def and_other_error_messages(self, other_error_messages: list[str]):
        self.__other_error_messages.extend(other_error_messages)
        return self

    def and_keyword_argument(self, keyword_argument: dict[str, typing.Any]):
    ...
    def build(self) -> DyError:
        dy_error =  DyError(
            self.__error_id, 
            self.__error_entry.get("title", 'Error'), 
            self.__error_entry.get("error_message", 'Got an exception'), 
...
class ErrorDatacache(IDatacache):
    def __init__(self):
        ...
        self.__error_entries : dict[str,dict[str,dict[str,typing.Any]]]= {}

    @property
    def id(self) -> str: return DatacacheIdDef.ERROR.value

    def intialize(self):
        try:
            for error_data_file in  ConfigurationMgr.get_instance().walk_app_files("errcfg.toml"):
